...
sys.path.append('/data4/zhangye/noisyboundaries')
from ilib.utils import get_query_keys, get_query_keys_eval, enhance_op
...
class PixelContrastiveHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(PixelContrastiveHead, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(PixelContrastiveHead, self).init_weights()
    ...
    def forward(self, x, edges, masks):
        ...
        if masks is not None: #training phase
            sample_results = get_query_keys( edges, masks, thred_u=self.thred_u, scale_u=self.scale_u, percent=self.percent)
        ...
        # print("x enhance", x_enhance.shape)
        ...
        if masks is not None:
            ...
            #queries
            query_pos = (x_pro.reshape(-1, 28, 28, 64) * sample_results['query_pos_sets'].to(device=x_pro.device).unsqueeze(3)).sum(dim=[1,2]) / (query_pos_num.unsqueeze(1)+1e-4) # foreground query for each proposal
            query_neg = (x_pro.reshape(-1, 28, 28, 64) * sample_results['query_neg_sets'].to(device=x_pro.device).unsqueeze(3)).sum(dim=[1,2]) / (query_neg_num.unsqueeze(1)+1e-4)
    ...
    def INFOloss(self, query, pos_sets, neg_sets, tem):
        ...
        # print("pos sets", pos_sets)
        # print("neg sets", neg_sets)
    ...
    def loss(self, easy_pos=None, easy_neg=None, hard_pos=None, hard_neg=None, query_pos=None, query_neg=None, t_easy=1.0, t_hard=1.0):
        """
            easy_pos: [B, N, 256]
            easy_neg: [B, N, 256]
            hard_pos: [B, N, 256]
        ...
        # print("pos easy", loss_Qpos_easy)
        # print("pos hard", loss_Qpos_hard)
        # print("neg easy", loss_Qneg_easy)
        # print("neg hard", loss_Qneg_hard)

        # loss_contrast = torch.mean(loss_Qpos_easy + loss_Qpos_hard + alpha*loss_Qneg_easy + alpha*loss_Qneg_hard)
        loss_contrast = torch.mean(alpha * loss_Qpos_hard + alpha*loss_Qneg_hard)
        ...
        return loss_contrast * self.weight
        # return 0

if __name__ == '__main__':
    con_head = ContrastiveHead()
    x = torch.randn((1, 256, 50, 50))
    cams = torch.ones((1,50,50))
    mask = torch.Tensor(np.load("mask.npy").reshape(1,50,50))
    edge = torch.Tensor(np.load("edge.npy").reshape(1,50,50))


    y = con_head(x, edge, mask)
    print(y)
