...
if __name__ == '__main__':
    # args = parse_args()
    # rname = args.rname
    rname = "rcity.segm.json"
    
    gt = mmcv.load('/data4/zhangye/coco/annotations/instances_train2017.1@20-unlabeled.json')
    class_num = [0] * 91
    for i in range(80):
        class_num[gt['categories'][i]['id']] = i
    
    gtval = mmcv.load('/data4/zhangye/coco/annotations/instances_train2017.1@20.json')
    gt_num = [0] * 80
    ...
    aaa = mmcv.load("/data4/zhangye/noisyboundaries/labels/rcity.bbox.json")
